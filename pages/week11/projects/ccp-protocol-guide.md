...
본 문서는 이번 주에 배운 리눅스 네트워크 프로그래밍을 복습하기 위해, 가상의 메신저 앱 샤통(ChatOn)에서 사용할 통신 규약을 설계해 본 기록입니다.

샤통 통신 규약인 **Chaton Communication Protocol (CCP)**는 서버와 클라이언트가 데이터를 주고받을 때 사용할 구조체 기반 바이너리 프로토콜입니다.

핵심 목표는 복잡한 직렬화 설계보다 **구현 난이도를 낮추고, 서버와 클라이언트가 같은 규칙으로 직관적으로 통신하는 것**입니다. 이를 위해 모든 통신은 `ccp_packet`이라는 고정 크기 단일 구조체를 사용하도록 설계했습니다.
...
고정 크기의 패킷을 사용하므로 TCP 상에서 스트림 처리가 단순해진다. 메시지 타입마다 서로 다른 크기의 데이터를 여러 번 나누어 보내는 대신, 논리적으로 하나의 `ccp_packet` 단위로 송수신하면 된다.

TCP 특성상 실제 수신 시 데이터가 중간에서 나뉘어 도착할 수 있지만, `recv_exact()`가 `sizeof(ccp_packet)`만큼 누적해서 읽도록 만들면 항상 정해진 길이의 완성된 패킷을 얻을 수 있다. 또한 멀티캐릭터 타입 상수(`'MSG'`, `'LGN'` 등)를 사용하면 `switch` 문에서 메시지 종류가 바로 보여 구현과 디버깅이 직관적이다.

`payload`는 공용체(union)로 구성되어 있어 일반 문자열, 64바이트 슬라이스 배열, 채팅방 목록처럼 서로 다른 형태의 데이터를 같은 1024바이트 영역에서 다룰 수 있다. 이 덕분에 별도의 복잡한 파싱 과정 없이 메시지 타입에 맞는 멤버로 바로 접근할 수 있다.
...
CCP(Chaton Communication Protocol)는 같은 규약을 참고한 외부 클라이언트 및 서버 구현체와 매끄럽게 통신 호환을 이루는 것을 지향한다. 이를 위해 외부 개발자가 제작한 프로그램과 통신할 때 반드시 다음 호환성 수칙을 준수해야 한다.
...
- **회원 정보가 일치하지 않는 외부 클라이언트가 직접 로그인 없이 서버로 연결하여 메시지를 보내는 경우**:
  ...
  - 이 게스트 유저가 일반 채팅 메시지 패킷(`type = CCP_TYPE_MESSAGE`)을 보내면, 서버는 요청 헤더의 `room_id` 값을 파싱해 해당 채팅방에 참여 중인 모든 사용자에게 성공적으로 브로드캐스트하여 호환 채팅이 가능하도록 처리한다.
